        ...
        while True:
            ...
            # -----------------------------------------------------------------------------------------
                ...
                if mevcut_gorev == "TASK2_START":
                    ...
                    # Search for Green Marker (Class ID 4) - REAL TIME DETECTION PRIORITY
                    # 2 Laps (8 phases) around task2_search_center_x,y

                    # 1. Check for Green Marker in REAL-TIME (Live Camera)
                    live_green_found = False
                    ...
                    if detections:
                        ...
                        for i, cid in enumerate(cids):
                            # ID 4 is Green Marker (Buoy/Object)
                            # Checking [1, 4, 12] for broader green compatibility if needed, but sticking to 4 as requested.
                            if cid == 4 or cid in [1, 12]:
                                x1, y1, x2, y2 = map(int, coords[i])
                                cx = int((x1 + x2) / 2)
                                cy = int((y1 + y2) / 2)
                                ...
                                if not np.isnan(dist_m) and not np.isinf(dist_m) and 0.5 < dist_m < 8.0:
                                    ...
                                    obj_wx = robot_x + (dist_m * math.cos(obj_global_angle))
                                    obj_wy = robot_y + (dist_m * math.sin(obj_global_angle))

                                    # Found it!
                                    live_green_found = True
                                    print(f"{Fore.GREEN}[TASK2] GREEN MARKER DETECTED LIVE! Dist: {dist_m:.1f}m{Style.RESET_ALL}")

                                    # Update Memory manually to ensure it persists for the next state
                                    update_landmark_memory("GREEN", obj_wx, obj_wy)

                                    # Transition
                                    mevcut_gorev = "TASK2_GREEN_MARKER_FOUND"
                                    task2_search_center_x = obj_wx
                                    task2_search_center_y = obj_wy
                                    task2_search_phase = 0
                                    task2_search_laps = 0
                                    break # Stop checking other detections

                    if not live_green_found:
                        # Continue Pattern
                        R = 2.0
